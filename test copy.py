...
print("Otwarto strumień wideo." if cap.isOpened() else "Problem podczas otwierania strumienia wideo.")
    ...
    for eye, eye_cascade in [('right', right_eye), ('left', left_eye)]:
        for (x, y, w, h) in eye_cascade:
            ...
            if eye == 'right':
                ...
                lpred = [prediction]
                print(f"Pewność modelu dla oka: {prediction:.2f} ({status})")

            break

    if rpred[0] < 0.1 and lpred[0] < 0.1:
        score += 1
        cv2.putText(frame, "Zamknięte  <---", (10, height-20), font, 1, (255, 255, 255), 1, cv2.LINE_AA)
        if score > threshold + 1:
            score = threshold
    else:
        score = max(score - 1, 0)
        cv2.putText(frame, "Otwarte  <---", (10, height-20), font, 1, (255, 255, 255), 1, cv2.LINE_AA)

    cv2.putText(frame, f'sennosc:{score}', (400, height-20), font, 1, (255, 255, 255), 1, cv2.LINE_AA)
